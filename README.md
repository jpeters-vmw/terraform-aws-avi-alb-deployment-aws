...
| [null_resource.system_configuration](https://registry.terraform.io/providers/hashicorp/null/latest/docs/resources/resource) | resource |
...
| <a name="input_custom_ami"></a> [custom\_ami](#input\_custom\_ami) | The AMI ID of a custom controller AMI.  For internal use. | `string` | `null` | no |
...
| <a name="input_fips"></a> [fips](#input\_fips) | Enable FIPS mode on AWS.  Specify the S3 bucket and prefix of the controller package accessible by the controller. | <pre>object({<br>    enabled               = bool,<br>    s3_bucket             = string,<br>    s3_controller_package = string<br>  })</pre> | <pre>{<br>  "enabled": "false",<br>  "s3_bucket": "",<br>  "s3_controller_package": "/controller.pkg"<br>}</pre> | no |
